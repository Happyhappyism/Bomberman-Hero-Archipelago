...
GEM_CHECK_PATCH =[
    0x3C,0x18,0x80,0x05, # LUI	T8, 0x8005
...
def write_tokens(world:World, patch:BombHProcedurePatch):
    ...
    # write game options
    patch.write_token(APTokenTypes.WRITE,0xB864D0,bytes([world.options.adok_bombs.value]))
    patch.write_token(APTokenTypes.WRITE,0xB864D1,bytes([world.options.game_goal.value]))
    if world.options.death_link:
        patch.write_token(APTokenTypes.WRITE,0xB864D2,bytes([0x01]))
    if world.options.random_sounds:
        patch.write_token(APTokenTypes.WRITE,0xB864D3,bytes([0x01]))
    if world.options.random_sky:
        patch.write_token(APTokenTypes.WRITE,0xB864D4,bytes([0x01]))
    if world.options.random_music:
        patch.write_token(APTokenTypes.WRITE,0xB864D5,bytes([0x01]))
    patch.write_token(APTokenTypes.WRITE,0xB864D6,bytes([world.options.clear_points.value]))
    #if world.options.item_powerups:
    patch.write_token(APTokenTypes.WRITE,0xB864D7,bytes([item_powers]))
    #if world.options.item_powerups:
    #    patch.write_token(APTokenTypes.WRITE,0xB864D7,bytes([0x01]))
    if world.options.radio:
        patch.write_token(APTokenTypes.WRITE,0xB864D8,bytes([0x01]))
    patch.write_token(APTokenTypes.WRITE,0xB864D9,bytes([world.options.stage_total.value]))
    ...
    #Death Link
    patch.write_token(APTokenTypes.WRITE, 0x24978,bytes([0x3C,0x1C,0x80,0x16])) # LUI	GP, 0x8016 {80023D78}
    patch.write_token(APTokenTypes.WRITE, 0x24984,bytes([0xA3,0x80,0x52,0x44])) # SB	R0, 0x5244 (GP) [0x80165240]
    patch.write_token(APTokenTypes.WRITE, 0x2499C,bytes([0x00,0x1C,0xE0,0x24])) # AND	GP, R0, GP

    # Do not reset stage progress to first stage
    patch.write_token(APTokenTypes.WRITE, 0x25F3C,NOP)

    # Move between worlds
    patch.write_token(APTokenTypes.WRITE, 0x5257C, bytes([0x10,0x00,0x00,0x04])) # B 0x80060070
    patch.write_token(APTokenTypes.WRITE, 0x52594, bytes([0x34,0x02]))
    ...
    # Change Adok Display
    patch.write_token(APTokenTypes.WRITE, 0x6261B, bytes([world.options.adok_bombs.value]))
    patch.write_token(APTokenTypes.WRITE, 0x625C0, bytes([0x80,0x64,0x03,0x24]))

    # Jump to Stage Code; J 0x8004CCD0, NOP; return to 0x803354B0
    patch.write_token(APTokenTypes.WRITE, 0x1519EC, bytes([0x08,0x01,0x33,0x40, 0x00,0x00,0x00,0x00]))

    # Patch in Stage Code
    patch.write_token(APTokenTypes.WRITE, 0x4D8D0, bytes(LEVEL_PATCH))

    # Patch in gem patch 0x00023A6C
    patch.write_token(APTokenTypes.WRITE, 0x239A7, bytes([world.options.needed_gem.value])) # SLTI	AT, T3, 0x00XX
    patch.write_token(APTokenTypes.WRITE, 0x23A6C, bytes([0xA0,0x20,0x52,0x41])) # SB	R0, 0x5241 (AT)
    patch.write_token(APTokenTypes.WRITE, 0x239E4, bytes(GEM_CHECK_PATCH))
    

    # Jump to Stage Code; J 0x8004CDD0, NOP; return to 0x80333CC4
    patch.write_token(APTokenTypes.WRITE, 0x1501FC, bytes([0x08,0x01,0x33,0x74, 0x00,0x00,0x00,0x00]))
    patch.write_token(APTokenTypes.WRITE, 0x14FF8B, bytes([0x07])) # Do not change the clear point overlay to 0

    patch.write_token(APTokenTypes.WRITE, 0x4D9D0, bytes(STG_SEL_OVLAY_PATCH))
    # Write patch file
